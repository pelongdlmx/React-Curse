...
class ResultsPokemon extends Component {
  render() {
    ...
    console.log("dentro del map", info.filter);
    let showInfo,
      PokData2 = [];
    let infoShow = [];
    ...
    if (info.searchResults) {
      showInfo = info.initialInfo.map((currentValue, index) => {
        ...
        if (n !== -1) {
          ...
          return axios({ url: url, timeout: 100000 }).then(function(
            dataPokemon
          ) {
            PokData2 = dataPokemon.data;
            if (PokData2.sprites.front_default === null) {
              PokData2 = new Pokemon(
                PokData2.name,
                PokData2.types[0].type.name,
                PokData2.stats[5].base_stat,
                PokData2.stats[2].base_stat,
                { pokeballimg }
              );
            } else {
              PokData2 = new Pokemon(
                ...
                PokData2.stats[5].base_stat,
                PokData2.stats[2].base_stat,
                PokData2.sprites.front_default
              );
            }
